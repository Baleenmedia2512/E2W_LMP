...
export const mockLeads: Lead[] = [
  {
    ...
    customerRequirement: 'Looking for project management solution',
    assignedTo: {
      id: '2',
      name: 'John Doe',
      email: 'john@example.com',
  ...
  },
  {
    ...
    customerRequirement: 'Marketing automation platform',
    assignedTo: {
      id: '3',
      name: 'Jane Smith',
      email: 'jane@example.com',
  ...
  },
  {
    ...
    source: 'Referral',
    campaign: null,
    customerRequirement: 'CRM implementation required',
    assignedTo: {
      id: '2',
      name: 'John Doe',
      email: 'john@example.com',
...
// Initialize activity history for existing leads
export const initializeActivityHistory = () => {
  if (mockActivityHistory.length === 0) {
    // Add creation history for each lead
    mockLeads.forEach((lead) => {
      mockActivityHistory.push({
        id: generateId(),
        leadId: lead.id,
        action: 'created',
        performedBy: lead.assignedTo || { id: '1', name: 'System', email: 'system@example.com' },
...
export const getActivityHistory = (leadId: string): ActivityHistory[] => {
  return mockActivityHistory
    .filter(activity => activity.leadId === leadId)
    .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
};

export const addActivityHistory = (activityData: Omit<ActivityHistory, 'id' | 'createdAt'>): ActivityHistory => {
  const newActivity: ActivityHistory = {
    ...activityData,
    id: generateId(),
    createdAt: new Date(),
  };
  mockActivityHistory.push(newActivity);
  return newActivity;
};

export const getCallAttemptsForLead = (leadId: string): number => {
  return mockCallLogs.filter(call => call.leadId === leadId).length;
};
